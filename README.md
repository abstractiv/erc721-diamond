# ERC721 using Diamond Pattern

This is an ERC721 project utilizing the [Diamond pattern](https://github.com/ethereum/EIPs/issues/2535). The Diamond pattern has the following advantages:
- Proxies to functions residing in multiple implementation contracts/libraries (as opposed to proxy contracts being able to only call functions from one implementation contract).
- Gets around the 24KB contract size limit. Since proxy can call from any number of implementation contracts, each implementation contract can be smaller.
- Decouples storage from implementation logic. 
- Defines data structures using `structs` in libraries which can be used in implementation contracts.
- Upgradeability.
